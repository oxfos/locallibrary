...
class AuthorCreateTest(TestCase):
    ...
    def test_loggedin_but_no_permission(self):
        login = self.client.login(username='testuser1', password='12345')
        resp = self.client.get(reverse('author_create'))
        self.assertFalse(resp.status_code == 200)

    def test_resp_succesful_when_login_correct(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.get(reverse('author_create'))
    ...
    def test_correct_user_logged_in(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.get('/catalog/author/create/')
        self.assertEqual(str(resp.context['user']), 'testuser2')

    def test_initial_value(self):
        login = self.client.login(username = 'testuser2', password = '12345')
        resp = self.client.get('/catalog/author/create/')
        self.assertEqual(resp.context['form'].initial['date_of_death'], '05/01/2018')
